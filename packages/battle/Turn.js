...
sendCurTurnEmbed = (char, btl) => {
	...
	let statDesc = '';
	if (char.shrouded) {
		statDesc += `${getBar('hp', 0, char.maxhp)}???/???HP\n${getBar('mp', 0, char.maxmp)} ???/???${char.mpMeter[1]}`;
	} else {
		statDesc += `${getBar('hp', char.hp, char.maxhp)}${char.hp}/${char.maxhp}HP\n${getBar('mp', char.mp, char.maxmp)} ${char.mp}/${char.maxmp}${char.mpMeter[1]}`;
	}
	...
	if (btl.teams.length > 2) {
		...
		for (let i in btl.teams[op].members) {
			...
			if (c.hp <= 0) {
				...
				if (c.shrouded) {
					teamDesc += `${l}: ${n} _(???/???HP, ???/???${c.mpMeter[1]})_\n`;
				} else {
					teamDesc += `${l}: ${s}${n} _(${c.hp}/${c.maxhp}HP, ${c.mp}/${c.maxmp}${c.mpMeter[1]})_\n`;
				}
			}
		}
	}
	...
	for (let i in btl.teams[char.team].members) {
		...
		if (c.hp <= 0) {
			...
			if (c.shrouded) {
				myTeamDesc += `${l}: **?**${n} _(???/???HP, ???/???${c.mpMeter[1]})_\n`;
			} else {
				myTeamDesc += `${l}: ${s}${n} _(${c.hp}/${c.maxhp}HP, ${c.mp}/${c.maxmp}${c.mpMeter[1]})_\n`;
			}
		}
	}
					...
					if (parseInt(i.customId) != char.team) {
						...
						for (let i in btl.teams[btl.action.target[0]].members) {
							...
							if (c.hp <= 0) {
								...
								teamDesc += `${l}: ${s}${c.name} _(${c.hp}/${c.maxhp}HP, ${c.mp}/${c.maxmp}${c.mpMeter[1]})_\n`;
							}
						}
