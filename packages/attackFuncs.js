...
function attackEnemy(userName, oppName, userDefs, oppDefs, skillDefs, useEnergy, server, btl) {
	...
	if (skillDefs.type === "heal") {		
		...
		if (skillDefs.fullheal) {
			...
			embedText.targetText = `${userName} => ${oppName}`
			embedText.attackText = `${userName} used ${skillDefs.name}!`
			embedText.resultText = `${(oppDefs.id == userDefs.id) ? "They" : oppName} had their ${oppDefs.mpMeter[1]} restored by **${skillDefs.pow}**!\n${healedQuote}`
			...
			if (userDefs.mp <= 0) {
				...
				embedText.targetText = `${userName} => ${oppName}`
				embedText.attackText = `${userName} used ${skillDefs.name}!`
				embedText.resultText = `${userName} converted their ${oppDefs.mpMeter[1]} into HP for ${(oppDefs.id == userDefs.id) ? "Themselves" : oppName}!`
			}
		} else {
		...
		if (skillDefs.stealmp) {
			...
			embedText.resultText = `${userName} stole **${skillDefs.pow}${userDefs.mpMeter[1]}** from the target!`
			return embedText
		}
			...
			if (dmg[1] !== "miss" && dmg[1] !== "block" && dmg[1] !== "drain") {
				...
				if (userDefs.hp <= 0) {
					...
					if (charFuncs.hasPassive(oppDefs, "sacrifice")) {
						...
						finaltext += `, with ${oppDefs.name}'s HP being restored by ${userDefs.level} & ${oppDefs.mpMeter[1]} restored by **${userDefs.level/2}**`
					}
				...
				if (dmg[1] !== "miss" && dmg[1] !== "block" && dmg[1] !== "drain") {
					...
					if (oppDefs.hp <= 0 && dmg[1] != "repel") {
						...
						if (charFuncs.hasPassive(userDefs, "sacrifice")) {
							...
							finaltext += `, with ${userDefs.name}'s HP being restored by ${oppDefs.level} & ${userDefs.mpMeter[1]} restored by **${oppDefs.level/2}**`
						}
					...
					if (charFuncs.hasPassive(targDefs, "alterpain") && targDefs.hp >= 0 && !skillDefs.feint) {
						...
						finaltext += `\n${targDefs.name}'s ${painDefs.name} allowed them to gain **${mpGain}${targDefs.mpMeter[1]}** back!`;
					}
				}
			} else {
				var finaltext = ``;
		...
		if (mpSteal > 0) {
			...
			finaltext += `\n${userName} managed to steal **${mpSteal}${userDefs.mpMeter[1]}** from ${oppName}!`;
		}
	...
	if (skillDefs.target === 'allopposing') {
		...
		if (enmDefs.hp <= 0) {
			...
			if (charFuncs.hasPassive(userDefs, "sacrifice")) {
				...
				finaltext += `, with ${userDefs.name}'s HP being restored by ${enmDefs.level} & ${userDefs.mpMeter[1]} restored by **${enmDefs.level/2}**`
			}
